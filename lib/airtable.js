...
export async function getDropdownOptions() {
  // Service Lines are a stable, canonical set (they map 1:1 to Rate Schedule
  // service types and the n8n service-line logic), so we seed them.
  const baseServiceLines = ["Design Only", "Design + Fabrication", "Fabrication Only", "Fabrication + Installation"];

  // Industries (Client Tier), however, are derived PURELY from the real data in
  // Portfolio + Client References. These are the values n8n matches against when
  // selecting portfolio projects, so the dropdown must offer exactly what exists
  // in the DB — no hardcoded seed. (A hardcoded seed previously injected bogus
  // "Health" / "Mid-Market" options that don't exist in any record and don't
  // match portfolio projects.)
  if (!isConfigured) {
    return {
      serviceLines: baseServiceLines,
      clientTiers: ["Corporate", "Government", "Healthcare", "Non-Profit"],
    };
  }

  // Pull values from Portfolio, Client References, and Team Bios.
  ...
  const clientTierSet = new Set(); // start empty — industries come only from real data
