...
export async function createRfpRecord({ rfpName, fileUrl, industry, serviceLine }) {
  ...
  let record;
  try {
    record = await tables.rfpTracker.create(fields);
  } catch (err) {
    // If "Industry" field doesn't exist yet in Airtable, retry without it
    if ((err.error === "UNKNOWN_FIELD_NAME" || err.message?.includes("Unknown field name")) && fields["Industry"]) {
      console.warn("Industry field not found in Airtable — retrying without it");
...
export async function getDropdownOptions() {
  ...
  const baseClientTiers = ["Government", "Corporate", "Non-Profit", "Mid-Market"];
